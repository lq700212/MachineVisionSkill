...
### 4.1 重连活锁铁律（HuaJiVision V4.4.70 开机血泪，写任何重连前先对三件套）

以下三条凑齐即活锁——首次重连失败后，每次触发都落在退避窗内被丢弃，链路永久无人救；
现场症状极具迷惑性：重连日志停在"已连续 1 次"、设备一直显示未连接，但 ping 对端正常、
其他设备都自愈了唯独它没有：
1. **固定周期触发源**（如心跳线程 5 秒一轮）；
2. **按"上次执行完成时间"算的时间退避**（如"上次执行 5 秒内不再重连"）；
3. **离线时单次执行耗时可能超过触发周期**（读超时 2 秒＋重连两次建连各 2 秒＋等待，轻松超 5 秒）。

解法二选一（不要调参赌运气，把退避从 5 秒改小只是把活锁窗口挪个位置）：
- **固定周期的触发走旁路**：心跳周期本身就是天然限流，不需要再被时间退避卡；
  去重（执行中/排队中丢弃）必须保留，防重连堆积。业务/脚本高频失败仍走退避版限流。
- 或者**触发周期 ≥ 2×（退避＋最坏执行耗时）**，用不等式硬保证每次触发都落在窗外。

自查口诀：看到"固定周期"四个字，立刻问一句"周期里调的链路上有没有时间退避"；
看到"上次×秒内不再×"的退避，立刻问一句"谁会固定周期调它，周期比窗口长吗"。
...
11. 重连无活锁（见 §4.1）：凡有"固定周期触发"（心跳/巡检/定时重扫），逐条确认链路上没有"按完成时间算的时间退避"；
    有退避则固定周期触发走旁路（去重保留），或触发周期 ≥ 2×（退避＋最坏执行耗时）；
12. 通讯相关改动同步更新通讯文档与 CHANGELOG（含寄存器/指令表与版本号）。
