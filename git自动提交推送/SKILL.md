...
0. **分支识别前置（V4.4.8 血泪铁律，任何提交/推送/撤回判断之前必做）**：
   先 `git branch --show-current` + `git status -sb` 确认**当前分支与其上游**，
   之后一切"是否已推送/领先几个提交/要不要 force"的判断，参照系**只能是当前分支的
   对应远程**（`git log @{u}..HEAD`），**禁止拿 main 或其它分支当参照**——
   实测事故：在 Pro_Lin 分支上用 `origin/main..HEAD` 判断"V4.4.8 未推送"，
   实际该提交早已推到 origin/Pro_Lin，参照系一错结论全错。
   推送目标=当前分支的 upstream（用户切到哪个分支就推哪个，不问不猜，以命令输出为准）；
   撤回已推送提交做覆盖时，先 `git fetch` 再确认远程该分支 HEAD 与预期一致，
   然后才允许 `git push --force-with-lease origin <当前分支>`（禁用裸 --force）。
