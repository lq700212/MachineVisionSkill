...
本次改动 **1 个 skill**：`上位机通讯封装`（SKILL.md＋内 CHANGELOG 同步），根 CHANGELOG.md 顶部追加本条。

### 上位机通讯封装（Unreleased 追补，未升版）

- 新增 §4.1 重连活锁铁律（HuaJiVision V4.4.70 开机血泪）：固定周期触发＋按完成时间算的时间退避＋离线单次执行耗时超周期三条凑齐即活锁；解法二选一：固定周期触发走旁路（去重保留）或触发周期 ≥ 2×（退避＋最坏执行耗时）。
- 同步 §七自检清单：新增第 11 条重连无活锁，原第 11 条顺延为第 12 条。
...
fix(上位机通讯封装): 补§4.1重连活锁铁律+自检第11条
